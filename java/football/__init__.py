...
def runs():
    ...
    check50.run("java Grade").stdin("21").stdin("27").stdin("300").stdout("Go for it", timeout = 60)


@check50.check(compiles)
def kicks():
    """Football.java says "Kick" when down by 2 with 5 minutes left in the game"""
    check50.run("java Grade").stdin("21").stdin("23").stdin("300").stdout("Kick", timeout = 60)

@check50.check(compiles)
def kicks2():
    """Football.java says "Kick" when down by 6 with 2 minutes left in the game"""
    check50.run("java Grade").stdin("14").stdin("20").stdin("120").stdout("Kick", timeout = 60)


@check50.check(compiles)
def kicks3():
    """Football.java says "Kick" when up by 2 with 5 minutes left in the game"""
    check50.run("java Grade").stdin("21").stdin("19").stdin("300").stdout("Kick", timeout = 60)
