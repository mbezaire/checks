...
def makequarter():
    ...
    check50._api.run("java F1Client 2>&1").stdin("1 4").stdout("1/4\n").exit(0)
    
@check50.check(compiles)
def divide():
    """Fractions can be divided"""
    check50.run("javac F2Client.java")
    check50._api.run("java F2Client 2>&1").stdin("1 4 1 3").stdout("12/1\n").exit(0)
...
def multiply():
    ...
    check50._api.run("java F3Client 2>&1").stdin("1 4 1 3").stdout("1/12\n").exit(0)
...
def add():
    ...
    check50._api.run("java F4Client 2>&1").stdin("1 4 1 3").stdout("7/12\n").exit(0)

@check50.check(compiles)
def subtract():
    """Fractions can be subtracted"""
    check50.run("javac F5Client.java")
    check50._api.run("java F5Client 2>&1").stdin("1 3 1 4").stdout("1/12\n").exit(0)