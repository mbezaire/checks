...
def runs():
    ...
    check50.run("java TwoGrade").stdin("90").stdin("95").stdout("Student has an A", timeout = 60)
...
def runs60():
    ...
    check50.run("java TwoGrade").stdin("50").stdin("55").stdout("Student failed :(", timeout = 60)        

@check50.check(compiles)
def runs59():
    """TwoGrade.java gives a fail if at least one grade is below 20"""
    check50.run("java TwoGrade").stdin("19").stdin("100").stdout("Student failed :(", timeout = 60)        
 
@check50.check(compiles)
def runs89():
    """TwoGrade.java gives a double average for other grades"""
    check50.run("java TwoGrade").stdin("50").stdin("90").stdout("Student has a 70.0 average", timeout = 60)        
