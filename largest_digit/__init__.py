...
def compiles():
    ...
    check50.spawn("clang -o largest_digit largest_digit.c -lcs50 -lm").exit(0)

@check50.check("compiles")
def largest_digit_pos():
    """The largest digit of a positive number is returned"""
    check50.spawn("./largest_digit").stdin("28451").stdout("8\n").exit(0)
...
def largest_digit_tie():
    ...
    check50.spawn("./largest_digit").stdin("4444").stdout("4\n").exit(0)

@check50.check("compiles")
def largest_digit_zero():
    """Zero returns zero"""
    check50.spawn("./largest_digit").stdin("0").stdout("0\n").exit(0)